...
func (cm *CaseManager) CrashCaseRecvAck04() (err error) {
	env, err := models.NewTestEnv("./cases/CrashCaseRecvAck04.ENV", cm.UseMatrix, cm.EthEndPoint)
	if err != nil {
		return
	}
	defer func() {
	...
	// 1. 启动
	// 启动节点2,4,5,6
	cm.startNodes(env, N2, N4, N5, N6,
		// 启动节点3, RefundTransferRecevieAck
		N3.SetConditionQuit(&params.ConditionQuit{
