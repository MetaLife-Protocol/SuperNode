...
// CrashCase015 : only for local test
func (cm *CaseManager) CrashCase015() (err error) {
	env, err := models.NewTestEnv("./cases/CrashCase015.ENV", cm.UseMatrix, cm.EthEndPoint)
	if err != nil {
		return
	}
	defer func() {
	...
	cm.startNodes(env, n1, n2,
		n0.SetConditionQuit(&params.ConditionQuit{
			QuitEvent: "ReceiveAnnounceDisposedResponseAck",
		}))
	// 初始数据记录
