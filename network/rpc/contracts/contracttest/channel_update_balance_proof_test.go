...
func TestUpdateBalanceProofRight(t *testing.T) {
	...
	tx, err := env.TokenNetwork.PrepareSettle(partner.Auth, env.TokenAddress, self.Address, big.NewInt(0), utils.EmptyHash, 0, utils.EmptyHash, nil)
	...
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartner.TransferAmount, bpPartner.LocksRoot, bpPartner.Nonce, bpPartner.AdditionalHash, bpPartner.Signature)
	...
	depositSelf, balanceHashSelf, nonceSelf, err := env.TokenNetwork.GetChannelParticipantInfo(nil, env.TokenAddress, self.Address, partner.Address)
	...
	depositPartner, balanceHashPartner, noncePartner, err := env.TokenNetwork.GetChannelParticipantInfo(nil, env.TokenAddress, partner.Address, self.Address)
	...
	tx, err = env.TokenNetwork.Settle(self.Auth, env.TokenAddress, self.Address, big.NewInt(0), utils.EmptyHash, partner.Address, bpPartner.TransferAmount, bpPartner.LocksRoot)
	assertTxSuccess(t, nil, tx, err)
...
func TestUpdateBalanceProofException(t *testing.T) {
	...
	tx, err := env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartnerFake.TransferAmount, bpPartnerFake.LocksRoot, bpPartnerFake.Nonce, bpPartnerFake.AdditionalHash, bpPartnerFake.Signature)
	...
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, big.NewInt(0), utils.EmptyHash, 0, utils.EmptyHash, nil)
	assertTxFail(t, &count, tx, err)
	bpPartnerEmpty := createPartnerBalanceProof(self, partner, big.NewInt(0), utils.EmptyHash, utils.EmptyHash, 0)
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartnerEmpty.TransferAmount, bpPartnerEmpty.LocksRoot, bpPartnerEmpty.Nonce, bpPartnerEmpty.AdditionalHash, bpPartnerEmpty.Signature)
	assertTxFail(t, &count, tx, err)
...
func TestUpdateBalanceProofEdge(t *testing.T) {
	...
	// EmptyAddress
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, EmptyAccountAddress, bpPartner.TransferAmount, bpPartner.LocksRoot, bpPartner.Nonce, bpPartner.AdditionalHash, bpPartner.Signature)
	assertTxFail(t, &count, tx, err)
	// wrong transfer amount
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, big.NewInt(1), bpPartner.LocksRoot, bpPartner.Nonce, bpPartner.AdditionalHash, bpPartner.Signature)
	assertTxFail(t, &count, tx, err)
	// wrong locksroot
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartner.TransferAmount, common.HexToHash("0x123"), bpPartner.Nonce, bpPartner.AdditionalHash, bpPartner.Signature)
	assertTxFail(t, &count, tx, err)
	// wrong nonce
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartner.TransferAmount, bpPartner.LocksRoot, 3, bpPartner.AdditionalHash, bpPartner.Signature)
	assertTxFail(t, &count, tx, err)
	// wrong additional hash
	tx, err = env.TokenNetwork.UpdateBalanceProof(self.Auth, env.TokenAddress, partner.Address, bpPartner.TransferAmount, bpPartner.LocksRoot, bpPartner.Nonce, common.HexToHash("0x123"), nil)
	assertTxFail(t, &count, tx, err)
	// wrong signature
	...
	waitToSettle(self, partner)
	// settle
	tx, err = env.TokenNetwork.Settle(self.Auth, env.TokenAddress, self.Address, big.NewInt(0), utils.EmptyHash, partner.Address, big.NewInt(0), utils.EmptyHash)
	assertTxSuccess(t, nil, tx, err)
...
func TestUpdateBalanceProofDelegate(t *testing.T) {
	...
	tx, err = env.TokenNetwork.UpdateBalanceProofDelegate(third.Auth, env.TokenAddress, partner.Address, self.Address, bpPartnerSelf.TransferAmount, bpPartnerSelf.LocksRoot, bpPartnerSelf.Nonce, bpPartnerSelf.AdditionalHash, bpPartnerSelf.Signature, bpPartnerSelf.NonClosingSignature)
	assertTxFail(t, &count, tx, err)
	...
	// settle
	waitToSettle(self, partner)
	tx, err = env.TokenNetwork.Settle(self.Auth, env.TokenAddress, self.Address, big.NewInt(0), utils.EmptyHash, partner.Address, bpPartnerSelf.TransferAmount, bpPartnerSelf.LocksRoot)
	assertTxSuccess(t, nil, tx, err)
