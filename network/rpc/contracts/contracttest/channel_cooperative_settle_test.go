...
func TestCooperativeSettleRight(t *testing.T) {
	...
	tx, err := env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, cs.Participant1Balance, a2.Address, cs.Participant2Balance, cs.sign(a1.Key), cs.sign(a2.Key))
...
func TestCooperativeSettleEdge(t *testing.T) {
	...
	// edge balance
	// partner1_balance = -1
	tx, err := env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, big.NewInt(-1), a2.Address, balanceA2, signA1, signA2)
	assertTxFail(t, &count, tx, err)
	// partner2_balance = -1
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, balanceA1, a2.Address, big.NewInt(-1), signA1, signA2)
	assertTxFail(t, &count, tx, err)

	// edge address
	// partner1_address = "0x0"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, common.HexToAddress("0x0"), balanceA1, a2.Address, balanceA2, signA1, signA2)
	assertTxFail(t, &count, tx, err)
	// partner2_address = "0x0"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, balanceA1, common.HexToAddress("0x0"), balanceA2, signA1, signA2)
	assertTxFail(t, &count, tx, err)
	// partner1_address = "0x0000000000000000000000000000000000000000"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, EmptyAccountAddress, balanceA1, a2.Address, balanceA2, signA1, signA2)
	assertTxFail(t, &count, tx, err)
	// partner2_address = "0x0000000000000000000000000000000000000000"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, balanceA1, EmptyAccountAddress, balanceA2, signA1, signA2)
	assertTxFail(t, &count, tx, err)

	// edge sign
	// partner1_sign = "0x0"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, balanceA1, a2.Address, balanceA2, common.Hex2Bytes("0x0"), signA2)
	assertTxFail(t, &count, tx, err)
	// partner2_sign = "0x0"
	tx, err = env.TokenNetwork.CooperativeSettle(a3.Auth, env.TokenAddress, a1.Address, balanceA1, a2.Address, balanceA2, signA1, common.Hex2Bytes("0x0"))
	assertTxFail(t, &count, tx, err)
...
func TestCooperativeSettleAttack(t *testing.T) {
	...
	tx, err := env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, cs.Participant1Balance, a2.Address, cs.Participant2Balance, cs.sign(a3.Key), cs.sign(a2.Key))
	assertTxFail(t, &count, tx, err)
	tx, err = env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, cs.Participant1Balance, a2.Address, cs.Participant2Balance, cs.sign(a1.Key), cs.sign(a3.Key))
	assertTxFail(t, &count, tx, err)
	// 错误余额
	// faulty balance proof
	tx, err = env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, cs.Participant2Balance, a2.Address, cs.Participant1Balance, cs.sign(a3.Key), cs.sign(a2.Key))
	assertTxFail(t, &count, tx, err)
	wrongBalance := big.NewInt(cs.Participant1Balance.Int64() + 1)
	tx, err = env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, wrongBalance, a2.Address, cs.Participant2Balance, cs.sign(a1.Key), cs.sign(a2.Key))
	assertTxFail(t, &count, tx, err)
	tx, err = env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, cs.Participant1Balance, a2.Address, wrongBalance, cs.sign(a1.Key), cs.sign(a2.Key))
	assertTxFail(t, &count, tx, err)
	wrongBalance = big.NewInt(cs.Participant1Balance.Int64() - 1)
	tx, err = env.TokenNetwork.CooperativeSettle(
		a3.Auth, env.TokenAddress, a1.Address, wrongBalance, a2.Address, cs.Participant2Balance, cs.sign(a1.Key), cs.sign(a2.Key))
